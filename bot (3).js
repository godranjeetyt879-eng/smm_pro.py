...
const DEFAULTS = {
  ...
  API_KEY      : process.env.SMM_API_KEY || '590827ad7527917688b58d4c951628f4',
  ...
  REF_PCT      : 5,
  MIN_RECHARGE : 10,
  // GuruPay Gateway (auto-pay)
  GURU_PAY_KEY : process.env.GURU_PAY_KEY || 'gurue59a6519ef8ca843a4eef4e693836c',
  GURU_PAY_URL : process.env.GURU_PAY_URL || 'https://www.gurupaygateway.com/api/create-order',
  GURU_PAY_CHECK_URL: process.env.GURU_PAY_CHECK_URL || 'https://www.gurupaygateway.com/api/check-order',
  // Gmail (sirf fallback ke liye rakha gaya — ab use nahi hota)
...
function upiQR(amount, upiId, upiName) {
  ...
  return `https://quickchart.io/qr?text=${encodeURIComponent(d)}&size=400&dark=1a237e&light=f0f4ff&ecLevel=H&margin=2&format=png`;
}
...
// ──────────────────────────────────────────────
//  GURUPAY GATEWAY — Auto Payment Verify
// ──────────────────────────────────────────────

// Fallback local ref ID (sirf emergency mein use hoga)
function genRefId() {
  const now = new Date();
  ...
  return 'GP' + ts + crypto.randomBytes(4).toString('hex').toUpperCase();
}

// GuruPay — create order & get reference ID from gateway
async function createGuruPayOrder(amount, userId) {
  try {
    const params = new URLSearchParams();
    params.append('gurukey',       DEFAULTS.GURU_PAY_KEY);
    params.append('amount',        String(amount));
    params.append('customer_name', `User_${userId}`);
    params.append('customer_email',`user${userId}@smmbot.in`);
    const res = await axios({
      method : 'POST',
      url    : DEFAULTS.GURU_PAY_URL,
      data   : params,
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      timeout: 15000,
      validateStatus: () => true,
    });
    console.log('[GuruPay] CreateOrder HTTP:', res.status, '| Response:', JSON.stringify(res.data).slice(0, 300));
    const d = res.data;
    if (!d) return { ok: false, error: 'Empty response from GuruPay' };

    // Try multiple common response field patterns
    const orderId = d?.data?.order_id || d?.order_id || d?.data?.id ||
                    d?.id   || d?.reference_id || d?.ref_id || null;
    const payUrl  = d?.data?.payment_url  || d?.payment_url  ||
                    d?.data?.payment_link || d?.payment_link ||
                    d?.url  || d?.data?.url || null;

    if (!orderId) {
      console.error('[GuruPay] No order_id in response:', JSON.stringify(d));
      return { ok: false, error: `Gateway ne order ID nahi diya. Response: ${JSON.stringify(d).slice(0,200)}` };
    }
    return { ok: true, orderId: String(orderId), payUrl };
  } catch (e) {
    console.error('[GuruPay] createGuruPayOrder error:', e.message);
    return { ok: false, error: e.message };
  }
}

// GuruPay — check payment status
async function checkGuruPayStatus(orderId) {
  try {
    const params2 = new URLSearchParams();
    params2.append('gurukey',  DEFAULTS.GURU_PAY_KEY);
    params2.append('order_id', String(orderId));
    const res = await axios({
      method : 'POST',
      url    : DEFAULTS.GURU_PAY_CHECK_URL,
      data   : params2,
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      timeout: 15000,
      validateStatus: () => true,
    });
    console.log('[GuruPay] CheckStatus HTTP:', res.status, '| Response:', JSON.stringify(res.data).slice(0, 300));
    const d = res.data;
    if (!d) return { ok: false, isPaid: false, error: 'Empty response from GuruPay' };

    // Gather status field — handles various common patterns
    const rawStatus = String(d?.status || d?.data?.status || d?.payment_status || '').toLowerCase();
    // Success patterns Indian gateways use
    const isPaid = rawStatus === 'success'   || rawStatus === 'paid'    ||
                   rawStatus === 'completed' || rawStatus === 'captured' ||
                   rawStatus === '1'         || d?.status === true       ||
                   d?.data?.status === true;
    return { ok: true, isPaid, status: rawStatus || 'unknown', raw: d };
  } catch (e) {
    console.error('[GuruPay] checkGuruPayStatus error:', e.message);
    return { ok: false, isPaid: false, error: e.message };
  }
}

// Gmail IMAP verify — DISABLED (GuruPay gateway use hota hai ab)
// Purana code neeche hai sirf reference ke liye, call nahi hota
async function checkGatewayOrder_DISABLED(refId, amount) {
  return new Promise((resolve) => {
    ...
    imap.once('ready', () => {
      // [Gmail]/All Mail catches ALL tabs: Primary, Updates, Transactions, Promotions
      // INBOX only shows Primary tab — UPI payment emails often land in Updates tab
      imap.openBox('[Gmail]/All Mail', true, (err) => {
        if (err) { imap.end(); return resolve({ ok:false, isPaid:false, error:'Mailbox open failed: '+err.message }); }

        // Aaj ki date se search karo (IMAP SINCE sirf date support karta hai)
        // Manual format used — toLocaleDateString() gives inconsistent output across Node.js envs
        const _nowD = new Date();
        const _months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
        const sinceStr = `${String(_nowD.getDate()).padStart(2,'0')}-${_months[_nowD.getMonth()]}-${_nowD.getFullYear()}`;
        // Also include yesterday (for payments near midnight, or delayed emails)
        const _yd = new Date(_nowD); _yd.setDate(_yd.getDate()-1);
        const sinceYday = `${String(_yd.getDate()).padStart(2,'0')}-${_months[_yd.getMonth()]}-${_yd.getFullYear()}`;

        // Search from yesterday onwards — catches midnight edge cases + delayed emails
        imap.search(['ALL', ['SINCE', sinceYday]], (err2, results) => {
          if (err2 || !results || results.length === 0) {
            imap.end();
            console.log(`[Gmail] No emails found since ${sinceYday} — inbox empty or search failed`);
            return resolve({ ok:true, isPaid:false, status:'pending', error:'No recent emails found' });
          }

          console.log(`[Gmail] Found ${results.length} emails since ${sinceYday}, fetching last 200`);
          // Last 200 emails fetch karo — captures recent payment notifications
          const toFetch = results.slice(-200);
          const f = imap.fetch(toFetch, { bodies: '' });
          let found = false;
          let foundByRef = false; // Track if matched by refId (stricter)

          // Amount ke saare possible formats (GPay, PhonePe, Paytm, bank email styles)
          const amt = parseFloat(amount);
          const amtInt = Math.floor(amt);
          const amtFix2 = amt.toFixed(2); // e.g. "500.00"
          const amtPatterns = [
            `₹${amt}`, `₹${amtInt}`, `₹ ${amt}`, `₹ ${amtInt}`,
            `₹${amtFix2}`, `₹ ${amtFix2}`,
            `₹${amtInt}/-`, `₹ ${amtInt}/-`,
            `rs.${amt}`, `rs ${amt}`, `rs.${amtInt}`, `rs ${amtInt}`,
            `rs. ${amtInt}`, `rs.${amtFix2}`, `rs ${amtFix2}`,
            `rs/-${amtInt}`, `rs/- ${amtInt}`, `rs./-${amtInt}`,
            `inr ${amt}`, `inr${amt}`, `inr ${amtInt}`, `inr${amtInt}`,
            `inr ${amtFix2}`, `inr${amtFix2}`, `inr ${amtInt}.00`,
            ` ${amt} `, ` ${amtInt} `,
            `${amt}.00`, `${amtInt}.00`, amtFix2,
            `amount ${amt}`, `amount ${amtInt}`, `amount ${amtFix2}`,
            `rupees ${amtInt}`, `rupees ${amt}`,
            `paid ${amtInt}`, `paid ${amt}`, `paid ₹${amtInt}`,
            `debit of ${amtInt}`, `debit of ₹${amtInt}`,
            `debited ${amtInt}`, `debited ₹${amtInt}`,
          ].map(s => s.toLowerCase());
          ...
          f.on('message', (msg) => {
            ...
            msg.once('end', async () => {
              ...
              try {
                ...
                // ── PRIORITY 2: Amount + keyword + merchant (fallback) ──
                // UPI payment emails from GPay/PhonePe do NOT always include the transaction
                // note (tn/refId) — only the amount and merchant name appear reliably.
                // So we use: amount matched + success keyword + merchant name/UPI ID as verification.
                else if (amtOk && kwOk) {
                  const dbCheck = loadDB();
                  // Duplicate prevention — same amount already approved today?
                  ...
                  // Merchant name / UPI ID presence — ensures payment went to our UPI account
                  const upiNameCheck = (DEFAULTS.UPI_NAME || '').toLowerCase();
                  const upiIdPart = (DEFAULTS.UPI_ID || '').split('@')[0]; // e.g. "7393028514"
                  const merchantOk = (upiNameCheck.length > 2 && full.includes(upiNameCheck)) ||
                                     (upiIdPart.length >= 5 && full.includes(upiIdPart));
                  if (sameAmtApproved) {
                    console.log(`[Gmail] ⚠️ SKIPPED: Same amount ₹${amt} already approved today`);
                  } else if (merchantOk) {
                    found = true;
                    console.log(`[Gmail] ✅ MATCHED BY AMT+KW+MERCHANT (₹${amt}): ${refId}`);
                  } else if (amt <= 10) {
                    // Very small amount — allow even without merchant (risk is low)
                    found = true;
                    console.log(`[Gmail] ✅ MATCHED BY AMT+KW (small ₹${amt}, no merchant needed): ${refId}`);
                  } else {
                    console.log(`[Gmail] ⚠️ amt+kw matched ₹${amt} but merchant name/UPI not in email — SKIPPING`);
                  }
                }
...
bot.onText(/\/start(?:\s+(.+))?/, async (msg, match) => {
  ...
  // ── Welcome message — 3-tier: custom forward > text msg > default ──
  const wFwd = db.settings.welcomeFwd;
  if (wFwd && wFwd.fromChatId && wFwd.messageId) {
    // Tier 1: Admin ne custom emoji wala message forward kiya hai
    try {
      ...
      // copyMessage fail toh text fallback
      const wMsg = '<tg-emoji emoji-id="5188481279963715781">🚀</tg-emoji> <b>Boost Your Social Media with Us!</b>\n━━━━━━━━━━━━━━━━━━━━\n<tg-emoji emoji-id="5226656353744862682">🛒</tg-emoji> <b>Order Services</b> - High-quality SMM services.\n<tg-emoji emoji-id="6167763767963685151">💰</tg-emoji> <b>Add Funds</b> - Easy UPI recharges.\n📦 <b>Track Orders</b> - Real-time order status.\n<tg-emoji emoji-id="6267115986541877538">🔗</tg-emoji> <b>Refer &amp; Earn</b> - Get amazing commissions!\n<tg-emoji emoji-id="5307746710682869587">🎧</tg-emoji> <b>24/7 Support</b> - Always here to help.';
      await send(uid, `<tg-emoji emoji-id="6122872666706676614">🎉</tg-emoji> <b>Welcome to Fan SMM!</b> <tg-emoji emoji-id="6122872666706676614">🎉</tg-emoji>\n\n<tg-emoji emoji-id="6003746869669270383">✅</tg-emoji> Hello <b>${user.firstName}</b>! <tg-emoji emoji-id="5199734171588642424">👋</tg-emoji>\n\n${wMsg}`, MAIN);
    }
  } else if (C.welcomeMsg) {
    // Tier 2: Admin ne text welcome message set kiya hai (Settings → Welcome Msg → Text)
    // {name} ko user ke naam se replace karo
    const customWelcome = C.welcomeMsg.replace(/\{name\}/gi, user.firstName);
    await send(uid, customWelcome, MAIN);
  } else {
    // Tier 3: Default hardcoded welcome message
    const wMsg = '<tg-emoji emoji-id="5188481279963715781">🚀</tg-emoji> <b>Boost Your Social Media with Us!</b>\n━━━━━━━━━━━━━━━━━━━━\n<tg-emoji emoji-id="5226656353744862682">🛒</tg-emoji> <b>Order Services</b> - High-quality SMM services.\n<tg-emoji emoji-id="6167763767963685151">💰</tg-emoji> <b>Add Funds</b> - Easy UPI recharges.\n📦 <b>Track Orders</b> - Real-time order status.\n<tg-emoji emoji-id="6267115986541877538">🔗</tg-emoji> <b>Refer &amp; Earn</b> - Get amazing commissions!\n<tg-emoji emoji-id="5307746710682869587">🎧</tg-emoji> <b>24/7 Support</b> - Always here to help.';
    await send(uid, `<tg-emoji emoji-id="6122872666706676614">🎉</tg-emoji> <b>Welcome to Fan SMM!</b> <tg-emoji emoji-id="6122872666706676614">🎉</tg-emoji>\n\n<tg-emoji emoji-id="6003746869669270383">✅</tg-emoji> Hello <b>${user.firstName}</b>! <tg-emoji emoji-id="5199734171588642424">👋</tg-emoji>\n\n${wMsg}`, MAIN);
  }
});
...
async function handleStep(msg, db, user, state, C) {
  ...
  if (state.step==='payment_amount_gw') {
    ...
    // ✅ GuruPay Gateway se order create karo — ref_id gateway se aayega
    await send(uid, '⏳ Payment gateway se order create ho raha hai...');
    const gwRes = await createGuruPayOrder(amt, uid);
    if (!gwRes.ok) {
      console.error('[GW] GuruPay order creation failed:', gwRes.error);
      return send(uid,
        `❌ <b>Payment Gateway Error</b>\n\n${gwRes.error}\n\n💡 Please try again or use 🏦 Manual UPI.`,
        CANCEL
      );
    }
    const ref_id = gwRes.orderId; // Reference ID from GuruPay
    ...
    db2.payments[ref_id] = {
      ...
      gwPayUrl: gwRes.payUrl || null,  // Payment URL if provided by gateway
      status: 'pending', type: 'gateway',
      createdAt: Date.now(), expiresAt,
    };
    user2.paymentHistory.push(ref_id);
    log(db2, { type: 'gateway_payment_created', userId: uid, paymentId: ref_id, amount: amt, gateway: 'gurupay' });
    saveDB(db2); clearState(uid);

    const expireStr = new Date(expiresAt).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'});
    const caption =
        `<tg-emoji emoji-id="6152069549442208798">💳</tg-emoji> <b>Complete Your Payment</b>\n\n` +
        `💳 <b>GuruPay Gateway</b>\n\n` +
        `<tg-emoji emoji-id="5841276284155467413">💰</tg-emoji> Amount: <b>₹${amt}</b>\n` +
        `<tg-emoji emoji-id="5413667824042256375">🆔</tg-emoji> Ref ID: <code>${ref_id}</code>\n\n` +
        `<tg-emoji emoji-id="5262540380301191210">⏳</tg-emoji> Payment karke neeche button dabao.\n` +
        `⏱️ Auto expire in 10 minutes (${expireStr})`;
    const replyBtns = [];
    // Agar gateway ne payment URL diya to direct button dikhao
    if (gwRes.payUrl) {
      replyBtns.push([{ text: '💳 Pay Now (GuruPay)', url: gwRes.payUrl }]);
    }
    replyBtns.push([{ text: '✅ 𝐏𝐚𝐢𝐝 — Check Status', callback_data: `gw_check_${ref_id}` }]);
    const replyKb = { inline_keyboard: replyBtns };
    await send(uid, caption, { parse_mode: 'HTML', reply_markup: replyKb });
    return;
  }
...
bot.on('callback_query', async (q) => {
  ...
  if (data.startsWith('gw_check_')) {
    ...
    // ── Turant "Verifying" message bhejo — IMAP check se PEHLE ──
    await send(uid,
    ...
    const checkId = pay.gwOrderId || pay.txnId || pay.id;
    const chk = await checkGuruPayStatus(checkId);

    // ── GuruPay API error ──
    if (!chk.ok) {
      return send(uid,
        `⏳ <b>Thoda time lag raha hai...</b>\n\n` +
        `GuruPay se status check nahi ho paya. 1-2 minute baad dobara try karo. 🙏`
      );
    }
    ...
    if (chk.ok && chk.isPaid) {
      ...
      return send(uid,
        ...
        `💸 <b>₹${pay.amount}</b> has been added to your wallet!\n\n` +
        `┌─────────────────────┐\n` +
        `  💳 Balance   : <b>₹${user2.balance.toFixed(2)}</b>\n` +
        `  🆔 Ref ID    : <code>${pid}</code>\n` +
        `  🕐 Time      : ${new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'})}\n` +
        `└─────────────────────┘\n\n` +
        `You're all set! Start placing orders 🚀`,
        MAIN
      );
    }
    ...
    // ── Payment abhi tak nahi mila ──
    return send(uid,
      `⏳ <b>Payment abhi tak nahi aaya...</b>\n\n` +
  ...
  if (data.startsWith('gw_newqr_') ) {
    ...
    // ✅ GuruPay Gateway se naya order create karo
    await send(uid, '⏳ Naya payment order create ho raha hai...');
    const gwRes2 = await createGuruPayOrder(pay.amount, uid);
    if (!gwRes2.ok) {
      saveDB(db2);
      return send(uid, `❌ Gateway Error: ${gwRes2.error}\n\nPlease try again ya Manual UPI use karo.`);
    }
    const newPid = gwRes2.orderId;

    const expiresAt = Date.now() + 10 * 60 * 1000; // 10 min
    db2.payments[newPid] = {
      id: newPid, userId: String(uid), amount: pay.amount,
      gwOrderId: newPid, txnId: newPid,
      gwPayUrl: gwRes2.payUrl || null,
    ...
    const newCaption =
      `<tg-emoji emoji-id="6152069549442208798">💳</tg-emoji> <b>Complete Your Payment</b>\n\n` +
      `💳 <b>GuruPay Gateway</b>\n\n` +
      `<tg-emoji emoji-id="5841276284155467413">💰</tg-emoji> Amount: <b>₹${pay.amount}</b>\n` +
      `<tg-emoji emoji-id="5413667824042256375">🆔</tg-emoji> Ref ID: <code>${newPid}</code>\n\n` +
      `<tg-emoji emoji-id="5262540380301191210">⏳</tg-emoji> Payment karke neeche button dabao.\n` +
      `⏱️ Auto expire in 10 minutes (${expireStr})`;
    const newBtns = [];
    if (gwRes2.payUrl) newBtns.push([{ text: '💳 Pay Now (GuruPay)', url: gwRes2.payUrl }]);
    newBtns.push([{ text: '✅ 𝐏𝐚𝐢𝐝 — Check Status', callback_data: `gw_check_${newPid}` }]);
    await send(uid, newCaption, { parse_mode: 'HTML', reply_markup: { inline_keyboard: newBtns } });
    return;
  }
  ...
  if (data.startsWith('sp_save_')) {
    ...
    await send(parseInt(staffId),
      `🎉 <b>You have been added as Staff!</b>\n\n👑 Admin has granted you staff access to Fan SMM.\n🔐 Your permissions:\n${perms.map(p=>`• ${p}`).join('\n')||'None'}\n\n👇 Use the menu below to get started:`,
      STAFF_KB
    ).catch(()=>{});
    return;
  }
  ...
  if (data.startsWith('staff_del_')) {
    ...
    await send(parseInt(staffId),`⚠️ Your staff access to Fan SMM has been revoked by admin.`).catch(()=>{});
    return;
  }
...
// ──────────────────────────────────────────────
//  AUTO-EXPIRE GATEWAY PAYMENTS (5 min)
//  Har 60s mein pending gateway payments check
//  karo — expire ho chuke ko mark karo aur user
//  ko message bhejo.
// ──────────────────────────────────────────────
setInterval(async () => {
  try {
    ...
    let changed = false;
    for (const [pid, pay] of Object.entries(db.payments)) {
      if (pay.type !== 'gateway' || pay.status !== 'pending') continue;
      if (!pay.expiresAt || now <= pay.expiresAt) continue;
      // Payment expire ho gayi
      pay.status = 'expired';
      changed = true;
      console.log(`[AutoExpire] Payment ${pid} expired for user ${pay.userId}`);
    ...
    if (changed) saveDB(db);
  } catch(e) { console.error('[AutoExpire] err:', e.message); }
}, 60_000); // Har 60 second mein check
