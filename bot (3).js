  ...
  try {
    ...
    const res = await axios({
      ...
      data   : {
        ...
        callback_url : 'https://t.me/fansmm_bot',
      },
      headers: { 'Content-Type': 'application/json', 'X-Guru-Key': DEFAULTS.GURU_PAY_KEY },
      timeout: 15000,
      validateStatus: () => true,
    });
    console.log('[GuruPay] Create HTTP:', res.status, JSON.stringify(res.data).slice(0,300));
    const d = res.data;
    if (!d) return { ok: false, error: 'Empty response' };
    const orderId = d?.data?.order_id || d?.order_id || d?.data?.id || d?.id || reqOrderId;
    const payUrl  = d?.data?.payment_url || d?.payment_url || d?.data?.url || d?.url || null;

    // ── GuruPay payment page se actual QR image fetch karo ──
    let qrImageUrl = null;
    if (payUrl) {
      try {
        const pageRes = await axios.get(payUrl, {
          timeout: 12000,
          headers: { 'User-Agent': 'Mozilla/5.0' },
        });
        const html = String(pageRes.data || '');
        // GuruPay page me QR image dhundho — multiple patterns try karo
        const patterns = [
          /src=["']([^"']*qr[^"']*\.(?:png|jpg|jpeg|svg|webp))["']/i,
          /src=["'](data:image\/(?:png|jpeg|gif|webp);base64,[^"']+)["']/i,
          /<img[^>]+id=["'][^"']*qr[^"']*["'][^>]+src=["']([^"']+)["']/i,
          /<img[^>]+class=["'][^"']*qr[^"']*["'][^>]+src=["']([^"']+)["']/i,
          /qr[_-]?(?:image|img|code)['":\s]+["']([^"']+)["']/i,
          /["'](https?:\/\/[^"']*qr[^"']*\.(?:png|jpg))["']/i,
        ];
        for (const pat of patterns) {
          const m = html.match(pat);
          if (m && m[1]) { qrImageUrl = m[1]; break; }
        }
        if (qrImageUrl && qrImageUrl.startsWith('/')) {
          const base = new URL(payUrl);
          qrImageUrl = base.origin + qrImageUrl;
        }
        console.log('[GuruPay] QR image found:', qrImageUrl ? qrImageUrl.slice(0,80) : 'NOT FOUND');
      } catch(pe) {
        console.error('[GuruPay] Page fetch error:', pe.message);
      }
    }

    return { ok: true, orderId: String(orderId), payUrl, qrImageUrl };
  } catch(e) {
    console.error('[GuruPay] createOrder error:', e.message);
    return { ok: false, error: e.message };
  }
}
...
async function checkGuruPayStatus(orderId) {
  try {
    ...
    console.log('[GuruPay] Check HTTP:', res.status, JSON.stringify(res.data).slice(0,300));
    const d = res.data;
    ...
    const isPaid = ['success','paid','completed','captured','1'].includes(rawStatus);
    return { ok: true, isPaid, status: rawStatus || 'pending' };
  } catch(e) {
    console.error('[GuruPay] checkStatus error:', e.message);
...
async function handleStep(msg, db, user, state, C) {
  ...
  if (state.step==='payment_amount_gw') {
    ...
    // QR — GuruPay ke page se extract ki gayi actual QR image use karo
