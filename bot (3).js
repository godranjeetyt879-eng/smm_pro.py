...
const DEFAULTS = {
  ...
  MIN_RECHARGE : 10,
  // FamPay Gateway
  FAM_PAY_KEY      : 'FAM_2B2C703378760FF5C5C13959A3BF7D301C7FC0E8',
  FAM_PAY_URL      : 'https://fampaygateway.site/api/create_order.php',
  FAM_PAY_CHECK_URL: 'https://fampaygateway.site/api/check_order.php',
  // Gmail (backup)
...
async function initDB() {
  try {
    ...
    if (doc && doc.data) {
      _dbCache = mergeDefaults(doc.data);
      // Clear old/broken welcomeMsg so hardcoded custom emoji version is used
      if (_dbCache.settings.welcomeMsg) {
        _dbCache.settings.welcomeMsg = '';
        await DBModel.findByIdAndUpdate('main', { $set: { data: _dbCache } }, { upsert: true });
        console.log('🔄 Welcome message reset to default.');
      }
...
function upiQR(amount, upiId, upiName) {
  ...
  return `https://quickchart.io/qr?text=${encodeURIComponent(d)}&size=500&dark=000000&light=FFFFFF&ecLevel=H&margin=3&format=png`;
}
...
function gatewayQrImageUrl(amount, orderId) {
  ...
  return `https://quickchart.io/qr?text=${encodeURIComponent(upiStr)}&size=500&dark=1a237e&light=f0f4ff&ecLevel=H&margin=3&format=png`;
}
...
//  FAMPAY — Order Create
// ─────────────────────────────────
async function createFamPayOrder(amount, userId) {
  try {
    const res = await axios({
      method : 'GET',
      url    : DEFAULTS.FAM_PAY_URL,
      params : {
        amount : String(amount),
        api_key: DEFAULTS.FAM_PAY_KEY,
      },
      timeout: 15000,
      validateStatus: () => true,
    });
    console.log('[FamPay] Create HTTP:', res.status, JSON.stringify(res.data).slice(0,300));
    const d = res.data;
    if (!d) return { ok: false, error: 'Empty response' };
    if (d.status === 'error' || d.error) return { ok: false, error: d.message || d.error || 'Gateway error' };

    const orderId    = d?.order_id || d?.id || d?.data?.order_id || d?.data?.id || ('FAM' + Date.now() + '_' + userId);
    const payUrl     = d?.payment_url || d?.pay_url || d?.url || d?.data?.payment_url || null;
    const qrImageUrl = d?.qr_url || d?.qr_image || d?.qr || d?.data?.qr_url || null;
    const upiId      = d?.upi_id || d?.vpa || d?.data?.upi_id || null;

    console.log('[FamPay] orderId:', orderId, '| payUrl:', payUrl, '| qrUrl:', qrImageUrl);
    return { ok: true, orderId: String(orderId), payUrl, qrImageUrl, upiId };

  } catch(e) {
    console.error('[FamPay] createOrder error:', e.message);
    return { ok: false, error: e.message };
  }
}

// ─────────────────────────────────
//  FAMPAY — Check Payment Status
// ─────────────────────────────────
async function checkFamPayStatus(orderId) {
  try {
    const res = await axios({
      method : 'GET',
      url    : DEFAULTS.FAM_PAY_CHECK_URL,
      params : {
    ...
    console.log('[FamPay] Check HTTP:', res.status, JSON.stringify(res.data).slice(0,300));
    const d = res.data;
    if (!d) return { ok: false, isPaid: false, error: 'Empty response' };
    const rawStatus = String(d?.status || d?.payment_status || d?.data?.status || '').toLowerCase();
    const isPaid = ['success','paid','completed','captured','1','true'].includes(rawStatus);
    return { ok: true, isPaid, status: rawStatus || 'pending' };
  } catch(e) {
    console.error('[FamPay] checkStatus error:', e.message);
...
async function handleStep(msg, db, user, state, C) {
  ...
  if (state.step==='payment_amount_gw') {
    ...
    // FamPay se order create karo
    const gwRes = await createFamPayOrder(amt, uid);
    if (!gwRes.ok) {
      return send(uid,
        `❌ <b>Payment Gateway Error</b>\n\n` +
        `Gateway ne order ID nahi diya. Response: ${gwRes.error}\n\n` +
        `💡 Please try again or use 🏦 Manual UPI.`,
        CANCEL
      );
    }

    const ref_id    = gwRes.orderId;
    const payUrl    = gwRes.payUrl;
    ...
    // QR — FamPay se mili qrImageUrl use karo, fallback quickchart UPI QR
    const qrImageUrl = gwRes.qrImageUrl || gatewayQrImageUrl(amt, ref_id);

    const caption =
      `<tg-emoji emoji-id="6152069549442208798">💳</tg-emoji> <b>Complete Your Payment</b>\n\n` +
      `<tg-emoji emoji-id="5440410042773824003">📱</tg-emoji> Scan this QR using any UPI App\n\n` +
      ...
      `<tg-emoji emoji-id="5262540380301191210">⏳</tg-emoji> Waiting for payment...\n` +
      `⏱️ Auto expire in 10 minutes (${expireStr})`;

    const replyKb = { inline_keyboard: [
      [{ text: '✅ 𝐏𝐚𝐢𝐝', callback_data: `gw_check_${ref_id}` }],
    ]};
    let qrMsg = null;
    try {
      qrMsg = await bot.sendPhoto(uid, qrImageUrl, { caption, parse_mode: 'HTML', reply_markup: replyKb });
    } catch(e) {
      console.error('[QR send error]', e.message);
      qrMsg = await send(uid, caption, { parse_mode:'HTML', reply_markup: replyKb });
    }

    // ── QR message_id save karo (5-min auto-delete ke liye) ──
    if (qrMsg?.message_id) {
      const db3 = loadDB();
      if (db3.payments[ref_id]) {
        db3.payments[ref_id].qrMsgId = qrMsg.message_id;
        saveDB(db3);
      }
    }

    // ── 5 minute mein QR auto-delete + expire ──
    setTimeout(async () => {
      try {
        const db4 = loadDB();
        const pay4 = db4.payments[ref_id];
        if (!pay4 || pay4.status !== 'pending') return; // already approved/expired
        pay4.status = 'expired';
        saveDB(db4);
        if (qrMsg?.message_id) await bot.deleteMessage(uid, qrMsg.message_id).catch(()=>{});
        await send(uid,
          `⏰ <b>Payment Expired!</b>\n\n` +
          `💸 ₹${amt}\n🆔 <code>${ref_id}</code>\n\n` +
          `QR expire ho gaya. Dobara try karna ho toh Wallet → Add Funds karo. 🙏`
        );
      } catch(e2) { console.error('[QR AutoExpire]', e2.message); }
    }, 5 * 60 * 1000); // 5 minutes
...
bot.on('callback_query', async (q) => {
  ...
  if (data.startsWith('gw_check_')) {
    ...
    // ── Retry count track karo PEHLE ──
    pay.checkCount = (pay.checkCount || 0) + 1;

    // ── 5 baar click ke baad auto-fail ──
    if (pay.checkCount > 5) {
      pay.status = 'expired';
      saveDB(db);
      if (pay.qrMsgId) await bot.deleteMessage(uid, pay.qrMsgId).catch(()=>{});
      else { try { await bot.deleteMessage(uid, q.message.message_id); } catch{} }
      return send(uid,
        `❌ <b>Payment Failed!</b>\n\n` +
        `5 baar verify kiya par payment nahi mili.\n\n` +
        `Agar paise gaye hain toh 🎧 Support se contact karo. 🙏`
      );
    }
    ...
    // ── Verifying message ──
    await send(uid,
      `🔍 Verifying... (${pay.checkCount}/5)\n⏳ Please wait...`
    );

    const checkId = pay.gwOrderId || pay.txnId || pay.id;

    // ── Step 1: FamPay API check ──
    let isPaid = false;
    const chk = await checkFamPayStatus(checkId);
    if (chk.ok && chk.isPaid) isPaid = true;

    // ── Step 2: Gmail fallback (agar FamPay ne confirm nahi kiya) ──
    if (!isPaid) {
      console.log(`[GW_CHECK] FamPay status="${chk.status}" → trying Gmail fallback for pid=${pid}`);
      try {
        const gmailChk = await checkGatewayOrder(pid, pay.amount);
        if (gmailChk.ok && gmailChk.isPaid) {
          isPaid = true;
          console.log(`[GW_CHECK] Gmail fallback: PAID ✅ pid=${pid}`);
        }
      } catch(ge) {
        console.log('[GW_CHECK] Gmail fallback error:', ge.message);
      }
    }
    ...
    return send(uid,
      ...
      `📊 Status : ⏳ <i>Pending</i>\n\n` +
      `━━━━━━━━━━━━━━━━━━━━━━━━\n` +
      `Agar payment ho gayi hai toh\n` +
      `✅ <b>Paid</b> dabao aur try karo 🙏\n` +
      `━━━━━━━━━━━━━━━━━━━━━━━━`
    );
  }
    ...
    try {
      await bot.sendPhoto(uid, qrImageUrl, {
        caption:
          ...
          `<tg-emoji emoji-id="5413667824042256375">🆔</tg-emoji> Ref: <code>${newPid}</code>\n\n` +
          `<tg-emoji emoji-id="5262540380301191210">⏳</tg-emoji> Waiting for payment...\n` +
          `⏱️ Auto expire in 5 minutes (${expireStr})`,
        parse_mode: 'HTML',
        reply_markup: { inline_keyboard: [
          [{ text: '✅ 𝐏𝐚𝐢𝐝', callback_data: `gw_check_${newPid}` }],
        ]},
      });
    } catch {
      await send(uid,
