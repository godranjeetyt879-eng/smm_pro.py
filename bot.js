  ...
  if (data.startsWith('gw_check_')) {
    ...
    if (pay.expiresAt && Date.now() > pay.expiresAt) {
      ...
      return send(uid,
        ...
        `⏰ QR expire ho gaya, payment nahi mila.\n` +
        `Naya recharge karo wallet se.`,
        MAIN
      );
    }
    ...
    if (!chk.ok) {
      return send(uid,
        ...
        `💰 Amount: ₹${pay.amount}\n` +
        `🆔 Order ID: <code>${pid}</code>\n\n` +
        `🔄 Payment server se connect nahi ho paya.\n` +
        `Please 1-2 minute baad dobara ✅ Paid click karo.\n\n` +
        `⚠️ Agar balance nahi aaya toh Support se contact karo.`
      );
    }
    ...
    // ── Payment abhi tak nahi mila — retry allow karo ──
    return send(uid,
      `🔍 <b>Payment Not Found Yet</b>\n\n` +
      `💰 Amount: ₹${pay.amount}\n` +
      `🆔 Order ID: <code>${pid}</code>\n\n` +
      `⏳ Payment Gmail mein nahi mila abhi.\n` +
      `Agar payment ho gayi hai toh 30-60 seconds ruko\n` +
      `aur dobara ✅ Paid click karo.\n\n` +
      `⚠️ Agar baar baar fail ho raha hai toh Support se contact karo.`
    );
  }
