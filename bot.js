...
//  KEYBOARDS
// ──────────────────────────────────────────────
// MAIN — inline keyboard (custom emoji supported)
const MAIN_INLINE = { reply_markup:{ inline_keyboard:[
  [
    { text:'<tg-emoji emoji-id="5417924076503062111">💰</tg-emoji> 𝐖𝐚𝐥𝐥𝐞𝐭',   callback_data:'main_wallet'   },
    { text:'<tg-emoji emoji-id="5431499171045581032">🛒</tg-emoji> 𝐍𝐞𝐰 𝐎𝐫𝐝𝐞𝐫', callback_data:'main_neworder' },
  ],
  [
    { text:'<tg-emoji emoji-id="5864114012542736772">📦</tg-emoji> 𝐌𝐲 𝐎𝐫𝐝𝐞𝐫𝐬', callback_data:'main_myorders' },
    { text:'<tg-emoji emoji-id="6267115986541877538">🔗</tg-emoji> 𝐑𝐞𝐟𝐞𝐫𝐫𝐚𝐥',  callback_data:'main_referral'  },
  ],
  [
    { text:'<tg-emoji emoji-id="5841697495893154141">📊</tg-emoji> 𝐌𝐲 𝐒𝐭𝐚𝐭𝐬', callback_data:'main_stats'   },
    { text:'<tg-emoji emoji-id="5307746710682869587">🎧</tg-emoji> 𝐒𝐮𝐩𝐩𝐨𝐫𝐭', callback_data:'main_support' },
  ],
  [
    { text:'<tg-emoji emoji-id="5334544901428229844">ℹ️</tg-emoji> 𝐇𝐞𝐥𝐩', callback_data:'main_help' },
  ],
]}}; 

// MAIN — kept as remove-keyboard (clears old reply keyboard)
const MAIN = { reply_markup:{ remove_keyboard: true }};
...
bot.onText(/\/start(?:\s+(.+))?/, async (msg, match) => {
  ...
  const wMsg = C.welcomeMsg || `<tg-emoji emoji-id="5188481279963715781">🚀</tg-emoji> <b>Boost Your Social Media with Us!</b>\n━━━━━━━━━━━━━━━━━━━━\n<tg-emoji emoji-id="5226656353744862682">🛒</tg-emoji> <b>Order Services</b> - High-quality SMM services.\n<tg-emoji emoji-id="6167763767963685151">💰</tg-emoji> <b>Add Funds</b> - Easy UPI recharges.\n📦 <b>Track Orders</b> - Real-time order status.\n<tg-emoji emoji-id="4958689671950369798">🔗</tg-emoji> <b>Refer & Earn</b> - Get amazing commissions!\n<tg-emoji emoji-id="5316919120149619748">🎧</tg-emoji> <b>24/7 Support</b> - Always here to help.`;
  await send(uid, `<tg-emoji emoji-id="6122872666706676614">🎉</tg-emoji> <b>Welcome to Fan SMM!</b> <tg-emoji emoji-id="5461151367559141950">🎉</tg-emoji>\n\n<tg-emoji emoji-id="6003746869669270383">✅</tg-emoji> Hello <b>${user.firstName}</b>! <tg-emoji emoji-id="5199734171588642424">👋</tg-emoji>\n\n${wMsg}`, MAIN_INLINE);
});
...
bot.on('message', async (msg) => {
  ...
  if (uid === DEFAULTS.ADMIN) {
    if (text==='🔙 𝐌𝐚𝐢𝐧 𝐌𝐞𝐧𝐮')    { clearState(uid); return send(uid,'🏠 <b>Main Menu</b>',MAIN_INLINE); }
  ...
  if (text==='❌ 𝐂𝐚𝐧𝐜𝐞𝐥' || text==='❌ Cancel') { clearState(uid); return send(uid,'❌ 𝐂𝐚𝐧𝐜𝐞𝐥ʟᴇᴅ.',MAIN_INLINE); }
  ...
  switch(text) {
    ...
    default:             return send(uid,'💡 Please use the menu buttons.',MAIN_INLINE);
  }
  } catch (e) {
...
async function handleStep(msg, db, user, state, C) {
  ...
  if (state.step==='order_confirm') {
    ...
    clearState(uid); return send(uid,'❌ Order cancelled.',MAIN_INLINE);
  }
  ...
  if (state.step==='ticket') {
    ...
    await send(uid,`🎟 <b>Ticket Created!</b>\n🆔 <code>${tid}</code>\n\n⏳ Our team will reply shortly.`,MAIN_INLINE);
  ...
  if (state.step==='a_set_apiurl') {
    ...
    const db2=loadDB(); db2.settings.apiUrl=text; saveDB(db2);
    // Clear service cache so users see fresh services from new API
    _svcCache = null; _svcCacheTs = 0;
    clearState(uid);
    return send(uid,`✅ API URL saved:\n<code>${text}</code>\n\n🔄 Service cache cleared — users will see fresh services.`,ADMIN_KB);
  }
  if (state.step==='a_set_apikey') {
    const db2=loadDB(); db2.settings.apiKey=text; saveDB(db2);
...
async function myOrders(uid,db,user,C) {
  if (!user.orderHistory.length) return send(uid,'📦 You have no orders yet.',MAIN_INLINE);
...
async function placeOrderFinal(uid,data,C) {
  ...
  if (!custom) {
    ...
    if (!res.ok || errFromBody) {
      ...
      return send(uid,`❌ <b>Order failed!</b>\n\nError: <code>${err}</code>\n\n💡 Please contact Support.`,MAIN_INLINE);
    }
...
bot.on('callback_query', async (q) => {
  ...
  if (data==='go_home') { clearState(uid); return send(uid,'🏠 <b>Main Menu</b>',MAIN_INLINE); }

  // ── Main inline menu callbacks ──
  if (data==='main_wallet')   { const u=getUser(db,uid); return walletMenu(uid,db,u,C); }
  if (data==='main_neworder') { return newOrder(uid,db,C); }
  if (data==='main_myorders') { const u=getUser(db,uid); return myOrders(uid,db,u,C); }
  if (data==='main_referral') { const u=getUser(db,uid); return referralMenu(uid,db,u,C); }
  if (data==='main_stats')    { const u=getUser(db,uid); return myStats(uid,db,u,C); }
  if (data==='main_support')  { return supportMenu(uid); }
  if (data==='main_help')     { return helpMenu(uid,C); }
  ...
  if (data.startsWith('ocat_')) {
    ...
    try {
      ...
      if (!svcsForCat || !svcsForCat.length) {
        ...
        const allSvcs=await getAllServices(db,C);
        if (!allSvcs.length) return send(uid,'❌ Could not load services. Please check API settings or try again.',MAIN_INLINE);
        const catsObj={};
        for (const s of allSvcs) { const c=s.category||'Other'; if(!catsObj[c]) catsObj[c]=[]; catsObj[c].push(s); }
      ...
      if (!svcsForCat || !svcsForCat.length) return send(uid,'❌ No services found in this category. Please tap 🔙 Back and try again.',MAIN_INLINE);
  ...
  if (data.startsWith('osvc_')) {
    ...
    try {
      ...
      if (!svc) {
        ...
        if (!allSvcs2.length) return send(uid,'❌ Could not load services. Please check API settings.',MAIN_INLINE);
        const cats2={};
      ...
      if (!svc) return send(uid,'⚠️ Service not found. Please tap 🛒 New Order and select again.',MAIN_INLINE);
