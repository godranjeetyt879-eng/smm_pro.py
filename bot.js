...
bot.on('callback_query', async (q) => {
  ...
  if (data.startsWith('ocat_')) {
    ...
    if (!svcsForCat || !svcsForCat.length) return send(uid,'❌ No services found in this category. Please go back and try again.',MAIN);
  ...
  if (data.startsWith('osvc_')) {
    ...
    if (!svc) return send(uid,'⚠️ Service could not be loaded. Please tap 🛒 New Order again.',MAIN);
