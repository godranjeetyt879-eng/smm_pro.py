...
//  GET ?api_key=KEY&amount=X&order_id=Y → returns QR image
//  CHECK: GET ?api_key=KEY&order_id=Y&amount=X → JSON status
// ──────────────────────────────────────────────
function gatewayQrImageUrl(amount, orderId) {
  return `${DEFAULTS.GATEWAY_URL}?api_key=${DEFAULTS.GATEWAY_KEY}&amount=${amount}&order_id=${encodeURIComponent(orderId)}`;
}

async function checkGatewayOrder(orderId, amount) {
  try {
    // Check via GET with order_id + amount
...
async function handleStep(msg, db, user, state, C) {
  ...
  if (state.step==='payment_amount_gw') {
    ...
    db2.payments[pid] = {
      ...
      status: 'pending', type: 'gateway', createdAt: Date.now(),
    };
    ...
    // Gateway directly QR image return karta hai
    const qrImageUrl = gatewayQrImageUrl(amt, pid);

    const caption =
      `💳 <b>Payment Complete Karo</b>\n\n` +
      `━━━━━━━━━━━━━━━━━━\n` +
      `💰 <b>Amount:</b>   ₹${amt}\n` +
      `🆔 <b>Order ID:</b> <code>${pid}</code>\n` +
      `━━━━━━━━━━━━━━━━━━\n\n` +
      `📲 <b>QR scan karo</b> aur pay karo\n` +
      `🏦 Ya UPI ID copy karo: <code>paytm.s1dw5n0@pty</code>\n\n` +
      `✅ Pay karne ke baad <b>Check Payment</b> dabao\n` +
      `⚡ Wallet automatically credit hoga`;
    ...
    try {
      ...
      console.log('[QR] Send failed:', e.message, '| URL:', qrImageUrl);
      await send(uid, caption + `\n\n🔗 <a href="${qrImageUrl}">Payment Link</a>`, {
        parse_mode: 'HTML',
        reply_markup: { inline_keyboard: [
          [{ text: '💳 Payment Page Kholo', url: qrImageUrl }],
          [{ text: '✅ Check Payment Status', callback_data: `gw_check_${pid}` }],
        ]},
      });
    }
