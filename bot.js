...
// Gmail IMAP verify — amount + keyword + ref_id teeno match hone chahiye
  ...
  return new Promise((resolve) => {
    ...
    imap.once('ready', () => {
      imap.openBox('INBOX', true, (err) => {
        ...
        // Last 30 minutes ke emails
        const since = new Date(Date.now() - 30 * 60 * 1000);
        const sinceStr = since.toISOString().slice(0,10); // YYYY-MM-DD
        imap.search(['ALL', ['SINCE', sinceStr]], (err2, results) => {
          if (err2 || !results || results.length === 0) {
            imap.end();
            return resolve({ ok:true, isPaid:false, status:'pending', error:'No recent emails' });
          }

          const toFetch = results.slice(-60); // last 60 emails
          ...
          f.on('message', (msg) => {
            ...
            msg.once('end', async () => {
              ...
              try {
                ...
                const full   = ((parsed.subject || '') + ' ' + (parsed.text || '') + ' ' + (parsed.html || '')).toLowerCase();

                const amtStr = Number.isInteger(amount) ? String(amount) : amount.toFixed(2);
                const amtOk  = [`rs.${amtStr}`,`rs ${amtStr}`,`inr ${amtStr}`,`₹${amtStr}`,
                                 ` ${amtStr} `,`${amtStr}.00`,`inr${amtStr}`].some(s => full.includes(s));
                const kwOk   = ['received','credited','credit','payment','upi','paid','success','transferred'].some(k => full.includes(k));
                const refOk  = full.includes(refId.toLowerCase());

                console.log(`[Gmail] subj="${parsed.subject?.slice(0,50)}" amt=${amtOk} kw=${kwOk} ref=${refOk}`);
          ...
          f.once('end', () => {
            imap.end();
            if (found) {
              resolve({ ok:true, isPaid:true, status:'success' });
            } else {
              resolve({ ok:true, isPaid:false, status:'pending' });
            }
          });
        });
      });
...
bot.on('message', async (msg) => {
  ...
  if (text==='❌ 𝐂𝐚𝐧𝐜𝐞𝐥') { clearState(uid); return send(uid,'❌ 𝐂𝐚𝐧𝐜𝐞𝐥ʟᴇᴅ.',MAIN); }
...
async function handleStep(msg, db, user, state, C) {
  ...
  if (state.step==='search_svc') {
    ...
    if (!q || q.length<2) return send(uid,'⚠️ Please type at least 2 characters to search.',CANCEL);
  ...
  if (state.step==='as_confirm') {
    if (text!=='✅ Confirm Add') { clearState(uid); return send(uid,'❌ 𝐂𝐚𝐧𝐜𝐞𝐥ʟᴇᴅ.',ADMIN_KB); }
    const db2=loadDB();
...
const PAGE_SIZE_CAT = 10; // categories per page in the alphabetical list
...
async function newOrder(uid, db, C, page = 0) {
  try {
    ...
    for (const plat of FIXED_PLATFORMS) {
      // Find the global index of the first matching category
      const matchIdx = catKeys.findIndex(c => plat.match.test(c));
      if (matchIdx !== -1) fixedBtns.push({ text: plat.label, callback_data: `ocat_${matchIdx}` });
    }
    ...
    if (totalPages > 1)       navRow.push({ text: `𝐍𝐞𝐱𝐭 ▶️`, callback_data: `ocat_page_${page + 1}` });
