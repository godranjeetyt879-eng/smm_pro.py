...
bot.on('callback_query', async (q) => {
  ...
  if (data.startsWith('gw_check_')) {
    ...
    // ── Gmail/IMAP connection error — silent, no response ──
    if (!chk.ok) {
      try { await bot.answerCallbackQuery(q.id); } catch{}
      return;
    }
    ...
    if (pay.checkCount < 3) {
      // Pehle 2 attempts — koi response nahi (silent)
      try { await bot.answerCallbackQuery(q.id); } catch{}
      return;
    }

    // 3rd attempt ke baad — Payment Failed, no buttons
    try { await bot.deleteMessage(uid, q.message.message_id); } catch{}
    return send(uid,
      `❌ <b>Payment Failed!</b>\n\n` +
      `💰 Amount: ₹${pay.amount}\n` +
      `🆔 Order ID: <code>${pid}</code>\n\n` +
      `⚠️ Payment confirm nahi hua.\n` +
      `Support se contact karo agar payment ho gayi hai.`,
      MAIN
    );
  }
