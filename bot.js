...
bot.on('callback_query', async (q) => {
  ...
  if (data.startsWith('gw_check_')) {
    ...
    if (pay.checkCount < 3) {
      // Pehle 2 attempts — koi response nahi (silent), bas answer callback query
      try { await bot.answerCallbackQuery(q.id); } catch{}
      return;
    }

    // 3rd attempt ke baad — Payment Failed dikhao
    try { await bot.deleteMessage(uid, q.message.message_id); } catch{}
    return send(uid,
      `❌ <b>Payment Failed!</b>\n\n` +
      `💰 Amount: ₹${pay.amount}\n` +
      `🆔 Order ID: <code>${pid}</code>\n\n` +
      `⚠️ 3 baar check karne ke baad bhi payment confirm nahi hua.\n\n` +
