...
async function handleStep(msg, db, user, state, C) {
  ...
  if (state.step==='payment_amount_gw') {
    ...
    // QR — GuruPay payUrl ka QR banao (scan karo toh GuruPay page khulega)
    const qrText     = payUrl || gatewayQrImageUrl(amt, ref_id);
    const qrImageUrl = payUrl
      ? `https://quickchart.io/qr?text=${encodeURIComponent(payUrl)}&size=500&dark=1a237e&light=f0f4ff&ecLevel=H&margin=3&format=png`
      : gatewayQrImageUrl(amt, ref_id);
