...
const DEFAULTS = {
  ...
  BOT_VERSION      : '4.0', // Official USDT TRC-20 contract
};
  ...
  if (data.startsWith('gw_check_')) {
    ...
    if (pay.expiresAt && Date.now() > pay.expiresAt) {
      ...
      return send(uid,
        ...
        `Dobara "Add Funds" karke naya QR lo.`,
        {reply_markup:{inline_keyboard:[
          [{text:'🔄 Naya QR Generate Karo', callback_data:'wallet_add'}],
        ]}}
      );
    }
    ...
    // ── Gateway "fail/invalid" = payment nahi hui, not an error ──
    if (!chk.ok) return send(uid,
      `⏳ <b>Payment Abhi Nahi Mili</b>\n\n` +
      `💰 ₹${pay.amount} | 🆔 <code>${pid}</code>\n\n` +
      `Pay karo phir check karo.`,
      {reply_markup:{inline_keyboard:[
    ...
    // ── Status = failed/invalid = user ne pay nahi kiya ──
    const statusMsg = ['failed','invalid','expired'].includes(chk.status)
      ? `❌ Payment nahi hui. QR scan karke pehle pay karo.`
      : `⏳ Abhi tak payment nahi mili. 1-2 min wait karo.`;
