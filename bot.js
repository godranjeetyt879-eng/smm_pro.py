...
// ──────────────────────────────────────────────
//  AUTO INSTALL — Must run FIRST before any require
// ──────────────────────────────────────────────
(function autoInstall() {
  const { spawnSync } = require('child_process');
  const path = require('path');

  const pkgs = ['node-telegram-bot-api', 'axios'];
  ...
  const r1 = spawnSync('npm', ['install', '--save', ...missing], {
    cwd: __dirname,
    stdio: 'inherit',
    shell: true,
  ...
  if (r1.status !== 0) {
    ...
    console.log('⚠️ Local install failed, trying alternative...');
    const r2 = spawnSync('npm', ['install', '-g', ...missing], {
      stdio: 'inherit',
      shell: true
    });
    if (r2.status !== 0) {
      console.error('❌ Failed to install packages. Make sure npm is available.');
      console.error('Run manually: npm install node-telegram-bot-api axios');
      process.exit(1);
    }
  }

  // Re-add local node_modules to path so require works immediately
  const nm = path.join(__dirname, 'node_modules');
  if (!require.main.paths.includes(nm)) {
