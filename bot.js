...
async function servicesList(uid,db,C) {
  try {
    ...
    if (!services.length)
      return send(uid,'❌ <b>No services available.</b>\n\n🔧 API might be down or key invalid.',{reply_markup:{inline_keyboard:[[{text:'🔄 Retry',callback_data:'services_retry'},{text:'🏠 Home',callback_data:'go_home'}]]}});
    ...
    btns.push([{text:'🔄 Refresh',callback_data:'services_retry'}]);
    await send(uid,`📊 <b>All Services</b>\n<i>${services.length} services in ${Object.keys(cats).length} categories</i>`,{reply_markup:{inline_keyboard:btns}});
  } catch(e) {
    console.error('[servicesList error]', e.message);
    return send(uid,`❌ Failed to load services.\n\n<code>${e.message}</code>`,{reply_markup:{inline_keyboard:[[{text:'🔄 Retry',callback_data:'services_retry'},{text:'🏠 Home',callback_data:'go_home'}]]}});
  }
}
...
bot.on('callback_query', async (q) => {
  ...
  if (data.startsWith('ocat_')) {
    ...
    try {
      ...
      if (!svcsForCat || !svcsForCat.length) {
        ...
        if (!allSvcs.length) return send(uid,'❌ Could not load services. Please check API settings (/testapi) or try again.',MAIN);
        const catsObj={};
  ...
  if (data==='services_retry') {
    _svcCache=null; _svcCacheTs=0;
    return servicesList(uid,db,C);
  }
  ...
  if (data.startsWith('scat_')) {
    try {
      const idx=parseInt(data.slice(5),10);
      const st=getState(uid);
      let cat=(st.sCatMap&&st.sCatMap[idx])||null;
      let services=await getAllServices(db,C);

      // State lost (bot restart) — rebuild from scratch
      if (!cat) {
        const cats0={};
        for (const s of services) { const c=s.category||'Other'; if(!cats0[c]) cats0[c]=[]; cats0[c].push(s); }
        const catKeys0=Object.keys(cats0).slice(0,15);
        cat=catKeys0[idx]||null;
        if (!cat) return servicesList(uid,db,C);
        const catMap0={}; catKeys0.forEach((c,i)=>catMap0[i]=c);
        setState(uid,{...st, sCatMap:catMap0});
      }

      const svcs=services.filter(s=>(s.category||'Other')===cat).slice(0,20);
      if (!svcs.length) return send(uid,'❌ No services found in this category.',{reply_markup:{inline_keyboard:[[{text:'🔙 Back',callback_data:'services_retry'}]]}});
      let txt=`📁 <b>${cat}</b>\n\n`;
      for (const s of svcs) txt+=`🆔 <code>${s.service}</code> — ${s.name.slice(0,40)}\n💵 ₹${addMargin(s.rate,C.margin)}/1K | ${s.min}-${s.max}\n\n`;
      return send(uid,txt,{reply_markup:{inline_keyboard:[[{text:'🔙 Back to Categories',callback_data:'services_retry'}]]}});
    } catch(e) {
      console.error('[scat_ error]', e.message);
      return send(uid,`❌ Failed to load category.\n<code>${e.message}</code>`,{reply_markup:{inline_keyboard:[[{text:'🔄 Retry',callback_data:'services_retry'}]]}});
    }
  }
