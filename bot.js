...
async function smmAPI(apiUrl, apiKey, params) {
  try {
    ...
    if (r.status === 403) return { ok:false, error:`Request failed with status code 403 — API Key is invalid or expired. Update it in ⚙️ Settings → API Key` };
    if (r.status === 404) return { ok:false, error:`Request failed with status code 404 — API URL is wrong. Update it in ⚙️ Settings → API URL` };
    if (r.status >= 400) return { ok:false, error:`Request failed with status code ${r.status}` };
    const d = r.data;
...
bot.onText(/\/testapi/, async (msg) => {
  ...
  await send(msg.chat.id, `🔍 <b>Testing API Connection...</b>\n\n🌐 URL: <code>${C.apiUrl}</code>\n🔑 Key: <code>${C.apiKey.slice(0,16)}...</code>`);
  const res = await smmAPI(C.apiUrl, C.apiKey, { action:'services' });
  if (res.ok && Array.isArray(res.data) && res.data.length>0) {
    const cats=[...new Set(res.data.map(s=>s.category||'Other'))];
    await send(msg.chat.id,
      `✅ <b>API Connected!</b>\n\n📊 Total Services: ${res.data.length}\n📂 Categories: ${cats.length}\n\n📁 Categories:\n${cats.slice(0,10).map(c=>`• ${c}`).join('\n')}\n\n💡 <b>Tip:</b> Copy a Service ID below, then go to:\n🛠 Services Mgmt → 📌 Add API Service by ID`,
      ADMIN_KB
    );
    // Send services in chunks so admin can copy IDs
    let chunk = '📋 <b>Services (copy ID to pin):</b>\n\n';
    let count = 0;
    for (const s of res.data.slice(0, 60)) {
      const line = `🆔 <code>${s.service}</code> — ${(s.name||'').slice(0,35)}\n📁 ${s.category||'Other'} | ₹${s.rate}/1K | ${s.min}-${s.max}\n\n`;
      if ((chunk + line).length > 3800) {
        await send(msg.chat.id, chunk);
        chunk = line;
      } else {
        chunk += line;
      }
    ...
    if (chunk.length > 50) await send(msg.chat.id, chunk);
    if (res.data.length > 60) await send(msg.chat.id, `📊 Showing first 60 of ${res.data.length} services.`, ADMIN_KB);
    return;
  }
  // Detailed error for debugging
  const statusHint = (res.error||'').includes('403') 
    ? '\n\n🔑 <b>403 = Wrong API Key!</b>\nLogin to your SMM panel → API section → copy fresh key.'
    : (res.error||'').includes('404')
    ? '\n\n🌐 <b>404 = Wrong API URL!</b>\nCheck your panel API URL (usually ends in /api/v2).'
    : '';
  return send(msg.chat.id,
    `❌ <b>API Connection Failed!</b>\n\n🌐 URL: <code>${C.apiUrl}</code>\n🔑 Key: <code>${C.apiKey}</code>\n\n⚠️ Error: <code>${res.error||'Unknown error'}</code>${statusHint}\n\n💡 <b>Fix steps:</b>\n1. Login to nixonsmm.com\n2. Go to API section\n3. Copy correct URL + Key\n4. Update in ⚙️ Settings → API URL / API Key\n5. Run /testapi again`,
    ADMIN_KB
  );
});
...
bot.on('callback_query', async (q) => {
  ...
  if (data==='cs_test_api' && uid===DEFAULTS.ADMIN) {
    ...
    if (res.ok&&Array.isArray(res.data)&&res.data.length>0) {
      const cats=[...new Set(res.data.map(s=>s.category||'Other'))];
      return send(uid,`✅ <b>API Connected!</b>\n📊 ${res.data.length} services | ${cats.length} categories\n\n${cats.slice(0,5).map(c=>`• ${c}`).join('\n')}\n\n💡 Use /testapi to browse services and get IDs to pin.`,ADMIN_KB);
    }
    const statusHint = (res.error||'').includes('403')
      ? '\n\n🔑 <b>403 = Wrong API Key!</b>\nLogin to your SMM panel → API section → copy fresh key → ⚙️ Settings → API Key'
      : (res.error||'').includes('404')
      ? '\n\n🌐 <b>404 = Wrong API URL!</b>\nCheck your panel API URL → ⚙️ Settings → API URL'
      : '';
    return send(uid,`❌ <b>API Connection Failed!</b>\n\n⚠️ Error: <code>${res.error||'Unknown'}</code>${statusHint}\n\n💡 Run /testapi for full details.`,ADMIN_KB);
  }
