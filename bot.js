...
// ──────────────────────────────────────────────
//  HARDCODED DEFAULTS (UPDATED WITH YOUR CREDENTIALS)
// ──────────────────────────────────────────────
const DEFAULTS = {
...
API_KEY      : process.env.SMM_API_KEY || 'bf22a5dc335b42b5e9ea6fe695028400',
...
// Staff Permissionsconst STAFF_PERMS = {
...
// If error contains "user not found" or similar, treat as NOT member// Only fail-open if it's a bot permission error (channel not found etc.)
...
['📊 Stats','👨‍💼 Staff Panel'],
['🔙 Main Menu'],
], resize_keyboard:true }};const STAFF_KB = { reply_markup:{ keyboard:[
['👥 Users','💳 Payments'],
['📦 Orders','🎟 Tickets'],
['📊 Stats','🔙 Main Menu'],
], resize_keyboard:true }};
...
if (msg.from.id !== DEFAULTS.ADMIN) return;
const db=loadDB();const input=(match[1]||'').trim();
if (!input) {
const C=cfg(db);
return send(msg.chat.id,
...
if (isStaff(db, uid)) {if (text==='🔙 Main Menu')    { clearState(uid); return send(uid,'🏠 Main Menu',MAIN); }
...
log(db2,{type:'broadcast',admin:uid,sent:ok,failed:fail}); saveDB(db2);return send(uid, `✅ Broadcast Done!\n📤 Sent: ${ok}\n❌ Failed: ${fail}`,ADMIN_KB);
}
...
const db2=loadDB(); const sid=state.data.sid;if (!db2.customServices[sid]) { clearState(uid); return send(uid,'❌ Service not found.',ADMIN_KB); }
...
async function adminBroadcast(uid) {setState(uid,{step:'a_broadcast',data:{}});
...
const activePinned=pinned.filter(s => s.active).length;await send(uid,
...
const comm=parseFloat((amt*C.refPct/100).toFixed(2));ref.balance+=comm; ref.referralEarnings+=comm;
...
reply_markup:{ inline_keyboard:[
[{ text:'📢 Join Channel Now', url:link }],[{ text:'✅ I Have Joined', callback_data:'check_join' }],
]}
}
);
} catch{}
...
if (!svcsForCat || !svcsForCat.length) {
  ...
  if (!catName) return newOrder(uid,db,C);  svcsForCat=(catsObj[catName]||[]).slice(0,20);
  // Rebuild STATE
...
if (data==='cs_edit_menu' && uid===DEFAULTS.ADMIN) {const svcs=Object.values(db.customServices);
if (!svcs.length) return send(uid,'❌ No custom services found.',ADMIN_KB);
...
let changed=false;for (const o of chunk) {
...
await send(uid,
`👨‍💼 <b>Staff Panel</b>\n\n✅ Welcome, <b>${sm.name}</b>!\n🔐 Your Permissions:\n${sm.perms.map(p => `• ${p}`).join('\n')}\n\n👇 Use the buttons below:`,STAFF_KB
);
});
// ──────────────────────────────────────────────
//  START
