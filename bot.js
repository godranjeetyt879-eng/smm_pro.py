...
async function newOrder(uid,db,C) {
  try {
    ...
    return send(uid,`❌ Failed to load services. Please try again.\n<code>${e.message}</code>`,MAIN);
  }
}
...
async function servicesList(uid,db,C) {
  ...
  setState(uid,{...getState(uid), sCatMap:catMap, sCatData:catData});
...
bot.on('callback_query', async (q) => {
  ...
  if (data.startsWith('scat_')) {
    const idx=parseInt(data.slice(5),10);
    const st=getState(uid);
    let catName=(st.sCatMap&&st.sCatMap[idx])||null;
    let svcs=null;
    // Try state first (fast)
    if (catName && st.sCatData && st.sCatData[idx]) {
      svcs=st.sCatData[idx].slice(0,30);
    }
    // Fallback: re-fetch
    if (!svcs || !svcs.length) {
      ...
      const catKeysArr=Object.keys(catsObj);
      catName=catKeysArr[idx]||null;
      if (!catName) return servicesList(uid,db,C);
      svcs=(catsObj[catName]||[]).slice(0,30);
      const newCatMap={}; catKeysArr.slice(0,15).forEach((c,i)=>newCatMap[i]=c);
      const newCatData={}; catKeysArr.slice(0,15).forEach((c,i)=>{ newCatData[i]=catsObj[c]; });
      setState(uid,{...st, sCatMap:newCatMap, sCatData:newCatData});
    }
    if (!svcs || !svcs.length) return send(uid,'❌ No services in this category.',MAIN);
    let txt=`📁 <b>${catName}</b>\n\n`;
    for (const s of svcs) txt+=`🆔 <code>${s.service}</code> — ${s.name.slice(0,40)}\n💵 ₹${addMargin(s.rate,C.margin)}/1K | Min: ${s.min} | Max: ${s.max}\n\n`;
    return send(uid,txt,{reply_markup:{inline_keyboard:[[{text:'🔙 Back',callback_data:'scat_back'}]]}});
  }
  if (data==='scat_back') return servicesList(uid,db,C);
      ...
      for (const o of chunk) {
        ...
        if (old!==o.status) {
          ...
          if (newStatus==='completed') {
            ...
            await sendLog(
              `❌ <b>Order Cancelled + Refunded (Auto)</b>\n\n` +
              `🆔 <code>${o.id}</code>\n` +
              `👤 User: <code>${o.userId}</code>\n` +
              `📌 ${o.serviceName}\n` +
              `💰 ₹${o.cost} refunded\n` +
