...
async function createGatewayOrder(amount, userId) {
  try {
    ...
    const res = await axios({
      method: 'POST',
      url: DEFAULTS.GATEWAY_URL,
      data: new URLSearchParams({
        key     : DEFAULTS.GATEWAY_KEY,
        action  : 'create',
    ...
    const d = res.data;
    if (d && (d.payment_url || d.url || d.payUrl || d.link)) {
      const payUrl = d.payment_url || d.url || d.payUrl || d.link;
      return { ok: true, orderId, payUrl, data: d };
    }
    if (d && d.error) return { ok: false, error: String(d.error) };
    // Some gateways return the URL directly as string
    if (typeof d === 'string' && d.startsWith('http')) return { ok: true, orderId, payUrl: d };
    console.log('[GATEWAY] Unexpected response:', JSON.stringify(d).slice(0, 300));
    return { ok: false, error: 'Unexpected gateway response', data: d };
  } catch (e) {
...
async function checkGatewayOrder(orderId) {
  try {
    const res = await axios({
      method: 'POST',
      url: DEFAULTS.GATEWAY_URL,
    ...
    const d = res.data;
    // status field can be: success / paid / completed / pending / failed
    const status = (d?.status || d?.payment_status || '').toLowerCase();
    const isPaid = ['success', 'paid', 'completed', 'approved'].includes(status);
    return { ok: true, isPaid, status, data: d };
  } catch (e) {
    return { ok: false, error: e.message };
  }
}
...
async function handleStep(msg, db, user, state, C) {
  ...
  if (state.step==='payment_amount_gw') {
    ...
    if (isNaN(amt)||amt<C.minRecharge) return send(uid,`❌ Minimum recharge is ₹${C.minRecharge}. Enter again:`,CANCEL);
    await send(uid,'⏳ Generating payment link...');
    const gw = await createGatewayOrder(amt, uid);
    if (!gw.ok) {
      // Fallback to UPI manual
      await send(uid,`⚠️ Gateway error (${gw.error}). Switching to manual UPI payment.`);
      try {
    ...
    // Save pending gateway order
    const pid='GPAY'+genId(8);
    const db2=loadDB(); const user2=getUser(db2,uid);
    db2.payments[pid]={id:pid,userId:String(uid),amount:amt,gwOrderId:gw.orderId,status:'pending',type:'gateway',createdAt:Date.now()};
    ...
    await send(uid,
      `💳 <b>Payment Link Ready!</b>\n\n💰 Amount: <b>₹${amt}</b>\n🆔 Order ID: <code>${pid}</code>\n\n👇 Click the button below to pay.\n\n⏳ Payment will be credited automatically after success.`,
      {reply_markup:{inline_keyboard:[
        [{text:`💳 Pay ₹${amt} Now`, url: gw.payUrl}],
        [{text:'✅ I Have Paid — Check Status', callback_data:`gw_check_${pid}`}],
      ]}}
    );
    return;
  }
...
bot.on('callback_query', async (q) => {
  ...
  if (data.startsWith('gw_check_')) {
    ...
    await send(uid,'⏳ Checking payment status...');
    const chk = await checkGatewayOrder(pay.gwOrderId);
    if (!chk.ok) return send(uid,`❌ Could not check status: ${chk.error}\n\nTry again in a moment.`);
    if (chk.isPaid) {
      // Auto approve
      ...
      saveDB(db);
      await adm(`✅ <b>Gateway Payment Auto-Approved</b>\n\n👤 ${user2.firstName} [<code>${uid}</code>]\n🆔 <code>${pid}</code>\n💰 ₹${pay.amount}`);
      return send(uid,`✅ <b>Payment Confirmed!</b>\n\n🆔 ${pid}\n💰 ₹${pay.amount} added to your wallet!\n💳 Balance: ₹${user2.balance.toFixed(2)}\n\nThank you! 🙏`, MAIN);
    }
    // Not paid yet
    ...
    return send(uid,
      ...
      {reply_markup:{inline_keyboard:[
        [{text:'🔄 Check Again', callback_data:`gw_check_${pid}`}],
      ]}}
    );
  }

  if (data==='wallet_add') { setState(uid,{step:'payment_amount_gw',data:{}}); return send(uid,`💳 Enter amount to recharge (min ₹${C.minRecharge}):`,CANCEL); }
  if (data==='wallet_hist') {
