...
// ──────────────────────────────────────────────
//  PAYMENT GATEWAY — Dynamic (uses GATEWAY_URL from env)
//  GET ?api_key=KEY&amount=X&order_id=Y → JSON with UPI string
//  QR: fetches UPI from new gateway API dynamically
// ──────────────────────────────────────────────

async function gatewayQrImageUrl(amount, orderId) {
  try {
    // Call new gateway to get UPI details dynamically
    const url = `${DEFAULTS.GATEWAY_URL}?api_key=${DEFAULTS.GATEWAY_KEY}&amount=${amount}&order_id=${encodeURIComponent(orderId)}`;
    console.log('[GATEWAY] Fetching UPI from:', url);
    const res = await axios.get(url, { timeout: 15000 });
    const d = res.data;
    console.log('[GATEWAY] UPI fetch response:', JSON.stringify(d));

    // Support multiple possible response formats from gateway
    const upiId   = d?.upi_id   || d?.upiid   || d?.vpa       || d?.pa;
    const upiName = d?.upi_name || d?.upiname  || d?.name      || d?.pn || 'Payment';
    const directQr= d?.qr_url   || d?.qrcode   || d?.qr_image;

    // If gateway returns a ready-made QR image URL, use it directly
    if (directQr) return directQr;

    // If gateway returns UPI ID, build QR from it
    if (upiId) {
      const upiStr = `upi://pay?pa=${upiId}&pn=${encodeURIComponent(upiName)}&am=${amount}&tr=${encodeURIComponent(orderId)}&tn=${encodeURIComponent('NEXUS SMM')}&cu=INR`;
      return `https://api.qrserver.com/v1/create-qr-code/?size=400x400&margin=10&data=${encodeURIComponent(upiStr)}`;
    }

    // If gateway returns full UPI string directly
    if (typeof d === 'string' && d.startsWith('upi://')) {
      return `https://api.qrserver.com/v1/create-qr-code/?size=400x400&margin=10&data=${encodeURIComponent(d)}`;
    }
    if (d?.upi_string && d.upi_string.startsWith('upi://')) {
      return `https://api.qrserver.com/v1/create-qr-code/?size=400x400&margin=10&data=${encodeURIComponent(d.upi_string)}`;
    }

    console.warn('[GATEWAY] Unrecognised gateway response — check gateway API format:', JSON.stringify(d));
    throw new Error('Gateway did not return a valid UPI ID or QR');
  } catch (e) {
    console.error('[GATEWAY] QR fetch error:', e.message);
    throw e; // Let caller handle — avoids silently showing wrong QR
  }
}
...
async function handleStep(msg, db, user, state, C) {
  ...
  if (state.step==='payment_amount_gw') {
    ...
    let qrImageUrl;
    try {
      qrImageUrl = await gatewayQrImageUrl(amt, pid);
    } catch (e) {
      // Gateway failed to return UPI — inform user and mark payment expired
      const db3 = loadDB();
      if (db3.payments[pid]) { db3.payments[pid].status = 'expired'; saveDB(db3); }
      return send(uid,
        `❌ <b>Payment Gateway Error</b>\n\n` +
        `Could not connect to payment gateway. Please try again or use Manual UPI.\n\n` +
        `<i>Error: ${e.message}</i>`,
        { reply_markup: { inline_keyboard: [
          [{ text: '🔄 Try Again', callback_data: 'wallet_add' }],
          [{ text: '🏦 Manual UPI', callback_data: 'wallet_manual' }],
        ]}}
      );
    }
...
bot.on('callback_query', async (q) => {
  ...
  if (data.startsWith('gw_newqr_') ) {
    ...
    let qrImageUrl;
    try {
      qrImageUrl = await gatewayQrImageUrl(pay.amount, newPid);
    } catch (e) {
      const db3 = loadDB();
      if (db3.payments[newPid]) { db3.payments[newPid].status = 'expired'; saveDB(db3); }
      return send(uid,
        `❌ <b>Gateway Error</b>\n\nCould not generate new QR. Please try again.\n<i>${e.message}</i>`,
        { reply_markup: { inline_keyboard: [[{ text: '🔄 Try Again', callback_data: 'wallet_add' }]] }}
      );
    }
