...
// CHECK payment status — GET format from vcapi
...
async function handleStep(msg, db, user, state, C) {
  ...
  if (state.step==='payment_amount_gw') {
    ...
    if (gw.payUrl) {
      // Payment link available — show button
      await send(uid,
        `💳 <b>Payment Link Ready!</b>\n\n💰 Amount: <b>₹${amt}</b>\n🆔 Order ID: <code>${pid}</code>\n\n👇 Click button below to pay.\n\n⏳ Wallet automatically credited after payment.`,
      ...
      // No redirect link — manual pay via UPI then check
      await send(uid,
        `💳 <b>Payment Created!</b>\n\n💰 Amount: <b>₹${amt}</b>\n🆔 Order ID: <code>${pid}</code>\n\nPlease complete UPI payment and tap Check Status.`,
        {reply_markup:{inline_keyboard:[
