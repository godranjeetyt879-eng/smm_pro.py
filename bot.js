...
bot.onText(/\/channels/, async (msg) => {
  ...
  if (!channels.length) {
    text += `❌ Koi channel set nahi hai.\n\n`;
  } else {
    channels.forEach((ch, i) => { text += `${i+1}. <code>${ch}</code>\n`; });
    text += `\nTotal: <b>${channels.length} channel(s)</b>\n\n`;
  }
  text += `📋 <b>Commands:</b>\n`;
  text += `• <code>/addchannel @username</code> — Channel add karo\n`;
  text += `• <code>/removechannel @username</code> — Channel hatao\n`;
  text += `• <code>/disablejoin</code> — Saare channels disable karo\n`;
  text += `• <code>/testjoin</code> — Test karo\n`;
...
bot.onText(/\/addchannel(?:\s+(.+))?/, async (msg, match) => {
  ...
  if (db.settings.forceChannels.includes(channel)) {
    return send(msg.chat.id, `⚠️ <code>${channel}</code> pehle se list mein hai!`, ADMIN_KB);
  }
  ...
  try {
    ...
    if (isAdminInCh) {
      await send(msg.chat.id,
        `✅ <b>Channel Add Ho Gaya!</b>\n\n📢 <code>${channel}</code>\n🤖 Bot: ✅ Admin\n\nTotal channels: <b>${db.settings.forceChannels.length}</b>\n\nSab users ko yeh channel join karna hoga.`,
        ADMIN_KB
      );
    } else {
      await send(msg.chat.id,
        `⚠️ <b>Channel save hua, lekin bot Admin nahi hai!</b>\n\n📢 <code>${channel}</code>\n🤖 Bot: ❌ NOT Admin\n\nForce join kaam nahi karega jab tak:\n1. <b>${channel}</b> kholo\n2. Administrators mein jao\n3. <b>@${botInfo.username}</b> ko Admin banao\n\n<code>/testjoin</code> se verify karo.`,
        ADMIN_KB
      );
    }
  } catch(e) {
    ...
    saveDB(db);
    await send(msg.chat.id,
      `⚠️ <b>Channel save hua (warning ke saath)</b>\n\n📢 <code>${channel}</code>\nError: <code>${e.message}</code>\n\nBot ko channel ka Admin banao aur <code>/testjoin</code> karo.`,
      ADMIN_KB
    );
  }
});
...
bot.onText(/\/removechannel(?:\s+(.+))?/, async (msg, match) => {
  ...
  if (!input) {
    ...
    let text = `📢 <b>Channel Hatao</b>\n\nUsage: <code>/removechannel @username</code>\n\n<b>Current channels:</b>\n`;
    if (!channels.length) text += `❌ Koi channel nahi hai.\n`;
  ...
  if (db.settings.forceChannels.length === before) {
    return send(msg.chat.id, `❌ <code>${channel}</code> list mein nahi mila.`, ADMIN_KB);
  }
  saveDB(db);
  await send(msg.chat.id,
    `✅ <b>Channel Hata Diya!</b>\n\n📢 <code>${channel}</code> remove ho gaya.\nBaki channels: <b>${db.settings.forceChannels.length}</b>`,
    ADMIN_KB
  );
});
...
bot.onText(/\/disablejoin/, async (msg) => {
  ...
  if (db.settings.forceChannel) delete db.settings.forceChannel;
  saveDB(db);
  await send(msg.chat.id, `✅ Force join <b>disable</b> ho gaya.\nSaare users ab freely bot use kar sakte hain.`, ADMIN_KB);
});
...
bot.onText(/\/setchannel(?:\s+(.+))?/, async (msg, match) => {
  ...
  if (!input || input === '0') {
    return send(msg.chat.id,
      `📢 <b>Updated Commands:</b>\n\n• <code>/addchannel @ch</code> — Channel add karo\n• <code>/removechannel @ch</code> — Channel hatao\n• <code>/channels</code> — Saare channels dekho\n• <code>/disablejoin</code> — Disable karo\n• <code>/testjoin</code> — Test karo`,
      ADMIN_KB
    );
  }
...
async function handleStep(msg, db, user, state, C) {
  ...
  if (state.step==='a_set_channel') {
    ...
    saveDB(db2); clearState(uid);
    return send(uid,`✅ Channel add ho gaya: ${ch}\nTotal: ${db2.settings.forceChannels.length} channel(s)\n\n/addchannel se aur bhi add kar sakte ho.`,ADMIN_KB);
  }
...
async function showChannelManager(uid, db, C) {
  ...
  if (!channels.length) {
    txt += `❌ Abhi koi channel set nahi hai.\n\n`;
  } else {
    channels.forEach((ch,i) => { txt += `${i+1}. <code>${ch}</code>\n`; });
    txt += `\n<b>Total: ${channels.length} channel(s)</b>\n\n`;
  }
  txt += `➕ Channel add karo ya ❌ hatao neeche se:`;

  const btns = [];
  // Delete button for each channel
  channels.forEach(ch => {
    btns.push([{ text:`❌ Hatao: ${ch}`, callback_data:`s_ch_del_${encodeURIComponent(ch)}` }]);
  });
  btns.push([{ text:'➕ Naya Channel Add Karo', callback_data:'s_ch_add' }]);
  if (channels.length > 0) {
    btns.push([{ text:'🚫 Sab Channels Disable Karo', callback_data:'s_ch_disable' }]);
  }
...
bot.on('callback_query', async (q) => {
  ...
  if (data==='check_join') {
    ...
    try {
      await bot.editMessageText(
        `❌ <b>Aapne channel join nahi kiya!</b>\n\n⚠️ Pehle <b>${notJoinedCh}</b> join karo, phir button dabao.\n\nBina join kiye bot use nahi kar sakte.`,
  ...
  if (data.startsWith('s_') && uid===DEFAULTS.ADMIN) {
    ...
    if (data==='s_ch_add')      { setState(uid,{step:'a_set_channel',data:{}}); return send(uid,'📢 Channel username bharo (e.g. @mychannel):

⚠️ <b>Important:</b> Pehle bot ko us channel ka Admin banao!',CANCEL); }
    if (data==='s_ch_disable')  { db.settings.forceChannels=[]; saveDB(db); return send(uid,'✅ Force join disable ho gaya. Saare users ab freely use kar sakte hain.',ADMIN_KB); }
