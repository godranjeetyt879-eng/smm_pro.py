...
function gatewayQR(amount, orderId) {
  // Gateway payment page — returns QR code image directly
  return `https://vcapi.vcstore.site/payment_api.php?api_key=${DEFAULTS.GATEWAY_KEY}&amount=${amount}&order_id=${orderId}`;
}
...
async function handleStep(msg, db, user, state, C) {
  ...
  if (state.step==='payment_amount_gw') {
    ...
    if (isNaN(amt) || amt < C.minRecharge)
      return send(uid, `❌ Minimum recharge amount is ₹${C.minRecharge}. Please enter again:`, CANCEL);

    await send(uid, '⏳ Creating your payment order...');
    ...
    db2.payments[pid] = {
      ...
      gwOrderId: pid, status: 'pending', type: 'gateway', createdAt: Date.now(),
    };
    ...
    // Gateway QR image URL
    const qrUrl = gatewayQR(amt, pid);
    ...
    const caption =
      ...
      `✅ After paying tap <b>Check Payment</b> below`;

    try {
      await bot.sendPhoto(uid, qrUrl, {
      ...
      // If QR image fails, send gateway link as button
      await send(uid, caption, { reply_markup: { inline_keyboard: [
        [{ text: `💳 Pay ₹${amt} — Open Payment Page`, url: qrUrl }],
        [{ text: '✅ Check Payment Status', callback_data: `gw_check_${pid}` }],
      ]}});
    }
    return;
