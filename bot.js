  ...
  if (data.startsWith('osvc_page_')) {
    ...
    try {
      ...
      if (!svcsForCat || !svcsForCat.length) {
        ...
        setState(uid, {...st, svcMap: svcMapR, svcPageData: svcsForCat, lastCatName: catName, lastCatIdx: catIdx});
      }
    ...
    try {
      ...
      if (!svc) {
        ...
        if (lastCat && cats2[lastCat]) {
          const svcs2=cats2[lastCat]; // ✅ FIXED: no slice limit
          svc=svcs2[idx]||null;
          const svcMap2={};
          svcs2.forEach((s,i)=>svcMap2[i]=s);
          setState(uid,{...getState(uid), svcMap:svcMap2, svcPageData:svcs2});
        }
      }
