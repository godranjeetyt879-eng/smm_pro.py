...
bot.on('callback_query', async (q) => {
  ...
  if (data.startsWith('ocat_')) {
    ...
    // FIX: Store full service objects in svcMap by index to avoid callback_data 64-char limit
    const svcMap={};
    svcs.forEach((s,i)=>svcMap[i]=s);
    setState(uid,{...getState(uid), svcMap});
  ...
  if (data.startsWith('osvc_')) {
    const idx=parseInt(data.slice(5),10);
    const st=getState(uid);
    // FIX: Get service from STATE svcMap by index - no more truncated ID mismatch
    const svc=(st.svcMap&&st.svcMap[idx])||null;
    if (!svc) return send(uid,'❌ Session timeout. Please tap 🛒 New Order again.',MAIN);
