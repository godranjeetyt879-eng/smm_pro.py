...
function mergeDefaults(data) {
  ...
  if (data.settings.autoBuy === undefined) data.settings.autoBuy = true;
...
function cfg(db) {
  ...
  return {
    ...
    autoBuy     : s.autoBuy !== undefined ? !!s.autoBuy : true,
  };
}
...
async function handleStep(msg, db, user, state, C) {
  ...
  if (state.step==='order_qty') {
    ...
    // Auto Buy ON hone pe confirm skip karke seedha order place
    if (C.autoBuy) {
      const cost=calcCost(state.data.rate,qty);
      setState(uid,{step:'order_confirm',data:{...state.data,qty,cost}});
      return placeOrderFinal(uid,{...state.data,qty,cost},C);
    }
...
async function adminSettings(uid,db,C) {
  await send(uid,
    ...
    {reply_markup:{inline_keyboard:[
      ...
      [{text:C.autoBuy?'🤖 Auto Buy: ON ✅ (tap to disable)':'🤖 Auto Buy: OFF ❌ (tap to enable)',callback_data:'s_toggle_autobuy'}],
    ]}}
  );
}
...
bot.on('callback_query', async (q) => {
  ...
  if (data.startsWith('s_') && uid===DEFAULTS.ADMIN) {
    ...
    if (data==='s_toggle_autobuy') {
      db.settings.autoBuy = db.settings.autoBuy===undefined ? false : !db.settings.autoBuy;
      saveDB(db); const C2=cfg(db);
      await bot.answerCallbackQuery(cq.id,{text: C2.autoBuy?'🤖 Auto Buy: ON':'🤖 Auto Buy: OFF'});
      return adminSettings(uid,db,C2);
    }
  }
