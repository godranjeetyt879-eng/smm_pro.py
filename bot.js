...
bot.on('callback_query', async (q) => {
  ...
  if (data.startsWith('gw_check_')) {
    ...
    if (pay.expiresAt && Date.now() > pay.expiresAt) {
      pay.status = 'expired'; saveDB(db);
      // Auto generate naya QR same amount se — user ko dobara amount enter na karna pade
      const newPid2 = 'GPAY' + genId(8);
      const expiresAt2 = Date.now() + 10 * 60 * 1000;
      const db3 = loadDB();
      db3.payments[newPid2] = {
        id: newPid2, userId: String(uid), amount: pay.amount,
        gwOrderId: newPid2, txnId: newPid2,
        status: 'pending', type: 'gateway',
        createdAt: Date.now(), expiresAt: expiresAt2,
      };
      getUser(db3, uid).paymentHistory.push(newPid2);
      saveDB(db3);
      const qrUrl2 = gatewayQrImageUrl(pay.amount, newPid2);
      const exp2 = new Date(expiresAt2).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'});
      try {
        await bot.sendPhoto(uid, qrUrl2, {
          caption:
            `⌛ <b>Purana QR Expire — Naya QR Ready!</b>\n\n` +
            `💰 Amount: <b>₹${pay.amount}</b>\n` +
            `🆔 Order: <code>${newPid2}</code>\n` +
            `⏰ Expires: <b>${exp2}</b> (10 min)\n\n` +
            `📲 QR scan karo aur pay karo`,
          parse_mode: 'HTML',
          reply_markup: { inline_keyboard: [
            [{ text: '✅ Check Payment Status', callback_data: `gw_check_${newPid2}` }],
          ]},
        });
      } catch {
        await send(uid,
          `⌛ Purana QR expire hua. Naya QR:\n\n💰 ₹${pay.amount}\n🆔 <code>${newPid2}</code>`,
          {reply_markup:{inline_keyboard:[[{text:'✅ Check Payment Status',callback_data:`gw_check_${newPid2}`}]]}}
        );
      }
      return;
    }
    ...
    // ── Payment nahi hui — FRESH QR generate karo naye order_id se ──
    // Purana order_id turant expire — gateway mein FAILED na aaye
    const newPid = 'GPAY' + genId(8);
    const expiresAt = Date.now() + 10 * 60 * 1000;
    const db2 = loadDB();
    // Expire old immediately
    if (db2.payments[pid]) {
    ...
    const user2 = getUser(db2, uid);
    user2.paymentHistory.push(newPid);
    saveDB(db2);
