...
// Generates plain black QR buffer (reliable — no URL, no Telegram download fail)
async function upiQrBuffer(amount, upiId, upiName) {
  const d = `upi://pay?pa=${upiId}&pn=${encodeURIComponent(upiName)}&am=${amount}&cu=INR`;
  return QRCode.toBuffer(d, {
...
// QR — generates buffer directly (plain black, reliable, no Telegram URL fetch issue)
async function gatewayQrImageBuffer(amount, orderId) {
  const C = cfg(loadDB());
  const upiStr = `upi://pay?pa=${C.upiId}&pn=${encodeURIComponent(C.upiName)}&am=${amount}&tn=${encodeURIComponent(orderId)}&cu=INR`;
  return QRCode.toBuffer(upiStr, {
    errorCorrectionLevel: 'H',
    width: 512,
    margin: 3,
    color: { dark: '#000000', light: '#FFFFFF' }
...
// Legacy URL fallback (not used in main flow)
function gatewayQrImageUrl(amount, orderId) {
  const C = cfg(loadDB());
  const upiStr = `upi://pay?pa=${C.upiId}&pn=${encodeURIComponent(C.upiName)}&am=${amount}&tn=${encodeURIComponent(orderId)}&cu=INR`;
  return `https://api.qrserver.com/v1/create-qr-code/?size=512x512&margin=20&qzone=2&ecc=H&color=000000&bgcolor=ffffff&data=${encodeURIComponent(upiStr)}`;
}
...
async function handleStep(msg, db, user, state, C) {
  ...
  if (state.step==='payment_amount_gw') {
    ...
    const qrBuffer = await gatewayQrImageBuffer(amt, ref_id);
    const expireStr = new Date(expiresAt).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'});
    const caption =
        `<tg-emoji emoji-id="6152069549442208798">₹</tg-emoji> <b>Deposit</b>\n\n` +
        `<tg-emoji emoji-id="5440410042773824003">🖼</tg-emoji> Scan this QR using any UPI App\n\n` +
        `<tg-emoji emoji-id="5841276284155467413">🆔</tg-emoji> Ref ID:\n` +
        `<code>${ref_id}</code>\n\n` +
        `<tg-emoji emoji-id="5413667824042256375">⏳</tg-emoji> Waiting for payment...\n` +
        `<tg-emoji emoji-id="5262540380301191210">⏰</tg-emoji> QR Expires in <b>5 minutes</b>\n\n` +
        `<tg-emoji emoji-id="6003746869669270383">✅</tg-emoji> Pay <b>₹${amt}</b> to <code>${C.upiId}</code>`;
    const replyKb = { inline_keyboard: [
        [{ text: '✅  𝗣𝗔𝗜𝗗', callback_data: `gw_check_${ref_id}` }],
    ]};
    try {
      await bot.sendPhoto(uid, qrBuffer, { caption, parse_mode: 'HTML', reply_markup: replyKb });
    } catch(e) {
      console.error('[QR send error]', e.message);
      // Fallback: text only with caption
  ...
  if (state.step==='payment_amount') {
    ...
    try {
      const manualQrBuf = await upiQrBuffer(amt, C.upiId, C.upiName);
      await bot.sendPhoto(uid, manualQrBuf, {
        caption:
          `💳 <b>UPI Payment</b>\n` +
          `━━━━━━━━━━━━━━━━━━━━\n` +
          `💰 Amount: <b>₹${amt}</b>\n` +
...
bot.on('callback_query', async (q) => {
  ...
  if (data.startsWith('gw_check_')) {
    ...
    // Track retry count (1st aur 2nd click silent, 3rd pe fail)
    pay.checkCount = (pay.checkCount || 0) + 1;
    saveDB(db);

    // 1st aur 2nd click pe koi response nahi — bilkul silent
    if (pay.checkCount < 3) {
      return; // No message, no response — user ko wait karne do
    }

    // 3rd click pe — Premium Payment Failed dikhao
    try { await bot.deleteMessage(uid, q.message.message_id); } catch{}
    return send(uid,
      `╔══════════════════════╗\n` +
      `       💳 𝗣𝗔𝗬𝗠𝗘𝗡𝗧 𝗙𝗔𝗜𝗟𝗘𝗗\n` +
  ...
  if (data.startsWith('gw_newqr_') ) {
    ...
    const newQrBuffer = await gatewayQrImageBuffer(pay.amount, newPid);
    const expireStr = new Date(expiresAt).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'});
    const Cfg = cfg(loadDB());

    try {
      await bot.sendPhoto(uid, newQrBuffer, {
        caption:
          `<tg-emoji emoji-id="6152069549442208798">₹</tg-emoji> <b>Deposit</b>\n\n` +
          `<tg-emoji emoji-id="5440410042773824003">🖼</tg-emoji> Scan this QR using any UPI App\n\n` +
          `<tg-emoji emoji-id="5841276284155467413">🆔</tg-emoji> Ref ID:\n` +
          ...
          `<tg-emoji emoji-id="5413667824042256375">⏳</tg-emoji> Waiting for payment...\n` +
          `<tg-emoji emoji-id="5262540380301191210">⏰</tg-emoji> QR Expires in <b>5 minutes</b>\n\n` +
          `<tg-emoji emoji-id="6003746869669270383">✅</tg-emoji> Pay <b>₹${pay.amount}</b> to <code>${Cfg.upiId}</code>`,
        parse_mode: 'HTML',
        reply_markup: { inline_keyboard: [
          [{ text: '✅  𝗣𝗔𝗜𝗗', callback_data: `gw_check_${newPid}` }],
        ]},
      });
    } catch {
      await send(uid,
        `🆕 New QR Code:\n💰 ₹${pay.amount}\n🆔 <code>${newPid}</code>\n⏰ Valid until ${expireStr}`,
        {reply_markup:{inline_keyboard:[[{text:'✅  𝗣𝗔𝗜𝗗',callback_data:`gw_check_${newPid}`}]]}}
      );
    }
    return;
  }
