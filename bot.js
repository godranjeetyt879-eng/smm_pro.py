...
const DEFAULTS = {
  ...
  API_KEY      : process.env.SMM_API_KEY || '657b8224885eaff4297ba7bc4f3edb16',
...
async function newOrder(uid,db,C) {
  ...
  // Encode category name safely as index — store full map in STATE
  const catMap={}; catKeys.forEach((c,i)=>catMap[i]=c);
  setState(uid,{...getState(uid), catMap, services});
...
bot.on('callback_query', async (q) => {
  ...
  if (data.startsWith('ocat_')) {
    ...
    // Always re-fetch services fresh — do NOT rely on STATE (lost on restart)
    const allSvcs=await getAllServices(db,C);
    const cats={};
    for (const s of allSvcs) { const c=s.category||'Other'; if(!cats[c]) cats[c]=[]; cats[c].push(s); }
    const catKeys=Object.keys(cats);
    const cat=catKeys[idx]||null;
    if (!cat) return newOrder(uid,db,C);
    const svcs=(cats[cat]||[]).slice(0,20);
    if (!svcs.length) return send(uid,'❌ No services in this category.');
    // Save svcMap in STATE by index
    const svcMap={};
    svcs.forEach((s,i)=>svcMap[i]=s);
    setState(uid,{...getState(uid), svcMap, lastCatIdx:idx});
    const btns=svcs.map((s,i)=>[{text:`${s.name.slice(0,40)} — ₹${addMargin(s.rate,C.margin)}/1K`,callback_data:`osvc_${i}`}]);
  ...
  if (data.startsWith('osvc_')) {
    ...
    if (!svc) {
      ...
      const catKeys2=Object.keys(cats2);
      const lastCat=catKeys2[st.lastCatIdx??-1]||null;
