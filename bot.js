...
(function bootstrap() {
  ...
  try {
    execFileSync(
      ...
      ['install', '--save', '--prefer-offline', '--no-audit', '--no-fund', ...missing],
    ...
    // Don't exit — packages might already be partially installed
    console.warn('⚠️ npm install warning (continuing anyway):', e.message);
    // Verify critical packages are actually available
    const stillMissing = needed.filter(pkg => {
      try { require.resolve(pkg, { paths: [__dirname] }); return false; }
      catch(_) { return true; }
    });
    if (stillMissing.length > 0) {
      console.error('❌ FATAL: Missing packages:', stillMissing.join(', '));
