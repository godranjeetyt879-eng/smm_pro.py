...
// Gmail IMAP verify — amount + keyword match (refId optional — UPI note mein nahi aata)
      ...
      imap.openBox('INBOX', true, (err) => {
        ...
        imap.search(['ALL', ['SINCE', sinceStr]], (err2, results) => {
          ...
          f.on('message', (msg) => {
            ...
            msg.once('end', async () => {
              ...
              try {
                ...
                // refId optional — agar email mein hai toh bonus, nahi toh sirf amt+kw kaafi
                const refOk = refId ? full.includes(refId.toLowerCase()) : false;

                console.log(`[Gmail] subj="${parsed.subject?.slice(0,60)}" | amt=${amtOk} kw=${kwOk} ref=${refOk}`);

                // Match: (amount + keyword) OR (amount + refId)
                if (amtOk && (kwOk || refOk)) {
                  found = true;
                }
          ...
          f.once('end', () => {
            imap.end();
            resolve(found
              ? { ok:true, isPaid:true,  status:'success' }
